// 1.1 Basandote en el array siguiente, crea una lista ul > li
// dinámicamente en el html que imprima cada uno de los paises.
// const countries = ['Japón', 'Nicaragua', 'Suiza', 'Australia', 'Venezuela']

const ul = document.createElement('ul')
document.body.appendChild(ul)

// for (const country of countries) {
//   const li = document.createElement('li')
//   li.innerHTML = country
//   ul.appendChild(li)
// }

// 1.2 Elimina el elemento que tenga la clase .fn-remove-me.

const pToBeRemoved = document.querySelector('.fn-remove-me')
pToBeRemoved.remove()

// 1.3 Utiliza el array para crear dinamicamente una lista ul > li de elementos en el div de html con el atributo data-function="printHere".
const cars = ['Mazda 6', 'Ford fiesta', 'Audi A4', 'Toyota corola']

const ul2 = document.createElement('ul')
document.body.appendChild(ul2)

for (const car of cars) {
  const li = document.createElement('li')
  li.innerHTML = car
  li.setAttribute('data-function', 'printHere')
  ul2.appendChild(li)
}

// 1.4 Crea dinamicamente en el html una serie de divs que contenga un elemento
// h4 para el titulo y otro elemento img para la imagen.
const countries = [
  { title: 'Random title', imgUrl: 'https://picsum.photos/300/200?random=1' },
  { title: 'Random title', imgUrl: 'https://picsum.photos/300/200?random=2' },
  { title: 'Random title', imgUrl: 'https://picsum.photos/300/200?random=3' },
  { title: 'Random title', imgUrl: 'https://picsum.photos/300/200?random=4' },
  { title: 'Random title', imgUrl: 'https://picsum.photos/300/200?random=5' }
]

const printCountries = () => {
  for (const country of countries) {
    const divCountries = document.createElement('div')
    divCountries.className = 'countries'
    document.body.appendChild(divCountries)
    const title = document.createElement('h4')
    const img = document.createElement('img')
    title.innerHTML = country.title
    img.src = country.imgUrl
    divCountries.appendChild(title)
    divCountries.appendChild(img)
  }
}

printCountries()

// 1.5 Basandote en el ejercicio anterior. Crea un botón que elimine el último
// elemento de la serie de divs.
const eliminarElemento = () => {
  const elementosAEliminar = document.querySelectorAll('.countries')
  elementosAEliminar.forEach((elemento) => {
    elemento.remove()
  })
  countries.pop()
  printCountries()
}
const boton = document.createElement('button')
boton.textContent = 'Eliminar último elemento'
document.body.appendChild(boton)
boton.addEventListener('click', eliminarElemento)
// 1.6 Basandote en el ejercicio anterior. Crea un botón para cada uno de los divs que elimine ese mismo elemento del html.

const addBotonEliminar = () => {
  const divCountries = document.querySelectorAll('.countries')

  for (const country of divCountries) {
    const botonEliminarDiv = document.createElement('button')
    botonEliminarDiv.className = 'boton-eliminar'
    country.appendChild(botonEliminarDiv)
    botonEliminarDiv.textContent = 'Eliminar este elemento'
    botonEliminarDiv.onclick = () => {
      var divPadre = botonEliminarDiv.parentNode
      divPadre.remove()
    }
  }
}
addBotonEliminar()

const botonEliminarDiv = document.querySelector('.boton-eliminar')
